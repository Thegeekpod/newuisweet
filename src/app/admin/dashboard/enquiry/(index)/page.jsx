...
import { deleteEnquiry, fetchEnquiry } from '@/app/admin/dashboard/enquiry/Function';
 ...
 const EnquiryList = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                const initialData = await fetchEnquiry();
                setData(initialData);
            } catch (error) {
                ...
                <table className="min-w-full bg-white border border-gray-300 rounded-md shadow-md">
                    <thead>
                        <tr>
                            ...
                            <th className="px-6 py-3 border-b-2 border-gray-300 text-left leading-4 text-blue-500 tracking-wider">Created</th>
                        ...
                        {currentData.length > 0 ? (
                            currentData.map((enquiry) => (
                                <tr key={enquiry.id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-500">{new Date(enquiry?.createdAt).toLocaleDateString()}</td>
