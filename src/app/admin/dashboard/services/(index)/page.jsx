...
import { deleteService, fetchServices } from '@/app/admin/dashboard/services/function';
const ServiceList = () => {
  noStore();
  ...
  useEffect(() => {
    ...
    const getServices = async () => {
      try {
        const initialServices = await fetchServices(); // Fetch initial data
    ...
    getServices();
  }, [fetchServices,deleteService,router]); // Empty dependency array ensures this runs once on mount
            ...
            {currentServices.length > 0 ? (
                currentServices.map((service) => (
                    <tr key={service?.id}>
                        ...
                        <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-500">{new Date(service?.createdAt).toLocaleDateString()}</td>
                        <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
