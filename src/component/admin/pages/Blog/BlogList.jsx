...
export const BlogList = ({faker}) => {
  ...
  useEffect(() => {
    ...
    getBlogs();
  }, [faker,deleteBlog,router]); // Empty dependency array ensures this runs once on mount
            ...
            {currentBlogs.length > 0 ? (
                currentBlogs.map((blog) => (
                    <tr key={blog?.id}>
                        ...
                        <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-500">{blog?.title.length > 20 ? `${blog?.title.slice(0, 20)}...` : blog?.title}</td>
